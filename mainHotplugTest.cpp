...
static int LIBUSB_CALL hotplug_callback(libusb_context *ctx, libusb_device *dev, libusb_hotplug_event event, void *user_data) {
    ...
    if (LIBUSB_SUCCESS != rc) {
        fprintf(stderr, "Error opening device; error: %i \n", rc);
    } else {
        rc = libusb_reset_device(handle);
        printf("Device %04x:%04x has been reset; result: %i\n", desc.idVendor, desc.idProduct, rc);
        rc = libusb_claim_interface(handle, 0);
        printf("Claimed interface 0; result: %i\n", rc);
    }
...
int main(int argc, char *argv[]) {
    ...
    vendor_id = (argc > 1) ? strtol(argv[1], NULL, 0) : LIBUSB_HOTPLUG_MATCH_ANY;
    product_id = (argc > 2) ? strtol(argv[2], NULL, 0) : LIBUSB_HOTPLUG_MATCH_ANY;
